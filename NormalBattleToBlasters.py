...
def hex_replacement(file, normalbattle_hex, blasters_hex):
    with open(file, 'rb') as file:
        content = file.read()

    content = content.replace(bytes.fromhex(normalbattle_hex), bytes.fromhex(blasters_hex))

    with open(file, 'wb') as file:
        file.write(content)

if len(sys.argv) != 2:
    print("""You have to use: python NormalBattleToBlasters.py [your_folder_name], ur nerd🤓""")
    sys.exit(1)
